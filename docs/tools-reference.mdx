...
1. **Structured Formatting**: Agents organize information with clear headings, lists, and emphasis
2. **Code Block Usage**: Agents present commands, code snippets, and configuration examples in code blocks
3. **Table Organization**: Agents present comparative or structured data in tables for clarity

### File Operations

1. **Path Safety**: Agents use absolute paths for file operations
2. **Error Handling**: Agents check for success or failure responses from file tools
3. **Minimize Writes**: Agents use `tool_forge_fs_patch` for small changes instead of rewriting entire files
4. **Content Verification**: Agents verify file content before modifying

### Shell Commands

1. **Command Safety**: Agents avoid potentially destructive commands
2. **Working Directory**: Agents specify the appropriate working directory
3. **Command Results**: Agents check command outputs and exit codes for errors

## Tool Selection Guide

| When agents need to... | They use this tool |
|---------------------|--------------|
...
| List files in a directory | `tool_forge_fs_list` |
| Get file metadata | `tool_forge_fs_info` |
