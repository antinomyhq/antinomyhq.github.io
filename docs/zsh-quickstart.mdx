...
Get Forge working in your ZSH shell in 3 simple steps. This guide will set up the core integration so you can use the `:` prefix to send prompts to Forge while keeping all your shell aliases, functions, and environment intact.
...
**Time required:** Quick setup with just 3 essential steps.
...
<Steps>
  ...
  <Step title="Verify Installation">
    ...
    You should see Forge responding to your prompts. 🎉
...
Enable quick file tagging with `@filename<Tab>` and fuzzy agent selection with `:<Tab>`.
...
If `@filename<Tab>` does nothing, fd and fzf are not installed. See [Fuzzy File Finding](#optional-enhancements) above or our [ZSH Dependencies Installation Guide](/docs/fd-fzf-installation).
