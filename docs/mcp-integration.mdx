...
- Find your path: Run `/info` in Forge Shell to see system paths (the configuration directory will be shown in the PATHS section)
...
> You can find the system paths by running `/info` in the Forge Shell.
...
The `get_weather` tool is now available to all your agents!
...
<img src="/images/mcp-tools-screenshot.png" alt="MCP Tools in Forge Shell" className="mcp-screenshot" />

This screenshot shows the `/tools` command output after adding our weather MCP server. Notice how the `get_weather` tool now appears in the list alongside Forge's built-in tools.
...
1. Check log files for MCP connection errors
