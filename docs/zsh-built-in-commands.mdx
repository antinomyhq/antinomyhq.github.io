...
```bash
:sage explain how authentication works
:muse plan OAuth integration
:forge implement the OAuth changes
```
...
:model             # Switch model
:reset             # Clear context
:dump html         # Export for debugging
:conversation      # Resume previous work
:new               # Start fresh
```

**Fuzzy Command Selection:**

Press Tab after `:` to search all commands and agents (requires fd/fzf).
...
- Use `:reset` when switching between unrelated tasks
- Use file tagging (`@file<Tab>`) for precision
- Use `:sage` for understanding, `:muse` for planning, `:forge` for coding
- Check `:info` regularly to monitor token usage
- For debugging use `:dump html`
...
- [Custom Agents](/docs/agent-definition-guide) - Create specialized agents
