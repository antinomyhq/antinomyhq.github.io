...
| `custom_rules`     | No       | Custom instructions for the agent                   |
...
| ----------- | --------------------------------------------------- |
| `subscribe` | List of command names the agent responds to         |
| `publish`   | (Advanced) Define custom channels for agent outputs |

## Custom Rules and Instructions

Provide special instructions or guidelines for specific agents:
...
agents:
  - id: security-analyst
    ...
    custom_rules: |
      ...
      - Always prioritize identifying security vulnerabilities
      - Check for common issues like XSS, CSRF, and SQL injection
      - Provide severity ratings for all identified issues
      - Suggest specific remediation steps for each vulnerability
```
...
### Code Generator (Precise, Deterministic)
...
agents:
  - id: coder
    ...
    system_prompt: "{{> coding-prompt.hbs }}"
    custom_rules: |
      Code Generation Rules:
      - Follow industry best practices for each language
      - Include comprehensive error handling
      - Add detailed comments for complex logic
      - Write unit tests for all functions
```

### Creative Writer (High Creativity)
...
agents:
  - id: creative-writer
    model: anthropic/claude-3.5-sonnet
    temperature: 1.2
    system_prompt: "{{> creative-writing-prompt.hbs }}"
```

### Background Helper Agent (Hidden)
...
agents:
  - id: research-assistant
    ...
    subscribe:
      - background_research
    system_prompt: "{{> research-assistant-prompt.hbs }}"
```
