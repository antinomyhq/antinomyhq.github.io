...
**Setting:** `forge.openTerminal`

Control whether to automatically open a Forge terminal when copying file references with `Ctrl+U`.
...
**Use `once` if:** You want the extension to automatically create and manage Forge terminals for you.

**Use `never` if:** You prefer to manually manage your own Forge terminal (whether internal or external) and only want the extension to copy file references to clipboard.

### Auto Paste

**Setting:** `forge.autoPaste`

Toggle automatic pasting of file references into Forge terminals.

- **`true`** (default): Automatically paste file references after the paste delay
- **`false`**: Only copy to clipboard, paste manually

**Enable if:** You want the extension to automatically paste file references into Forge terminals.

**Disable if:** You prefer to manually paste file references. The extension will still copy references to clipboard.

### Paste Delay

**Setting:** `forge.pasteDelay`

Set the delay (in milliseconds) to wait for Forge to start before auto-pasting file references.

- **Default**: `5000` (5 seconds)

**When this applies:** Only when the extension automatically creates a new Forge terminal (via `Ctrl+U` when no terminal exists, or "Start New Forge Session" command). Does not apply to manually started Forge terminals.

**Why this exists:** Forge needs time to start up before accepting input. This delay ensures the file reference is pasted after Forge is ready.

**Adjust this if:**
- File references aren't being pasted → Increase to 7000-10000ms
- You have a fast machine → Decrease to 3000-4000ms

### Path Format

**Setting:** `forge.pathFormat`

Choose between absolute or relative file paths when copying references.

- **`absolute`** (default): Full file paths (e.g., `/Users/name/project/src/file.ts`)
- **`relative`**: Workspace-relative paths (e.g., `src/file.ts`)

**Use `absolute` if:** You want unambiguous paths that work everywhere (recommended for most users).

**Use `relative` if:** You prefer shorter paths or work with multiple workspace folders.

**Tip:** Need a different format just once? Right-click in the editor and choose "Copy Absolute Path" or "Copy Relative Path" to override this setting for that specific copy.

### Installation Prompt

**Setting:** `forge.showInstallationPrompt`

Toggle the installation prompt when Forge CLI is not detected in your PATH.

- **`true`** (default): Show installation prompt
- **`false`**: Hide installation prompt

**Disable if:** You use Forge via `npx` or have it installed in a non-standard location.
