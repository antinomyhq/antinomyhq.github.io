...
- For Claude 3.5 Sonnet (~200K token window): 150,000 to 180,000 tokens
- For Claude 3 Opus (~180K token window): 120,000 to 160,000 tokens
- For GPT-4 Turbo (~128K token window): 80,000 to 100,000 tokens
...
You can provide a custom Handlebars template for the summarization prompt to better suit your specific use case:

```yaml
compact:
  prompt: "{{> custom-summarizer.hbs }}"
```
