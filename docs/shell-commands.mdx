...
Changing the directory will only be applicable until the command executes and will not affect Forge's working directory. The `cd` command or any directory navigation within a shell command is temporary and isolated to that specific command execution.
...
**Workaround**: To change Forge's working directory for a session, restart Forge in your target directory:
...
The `!` shortcut is for quick command execution without context switching, not as a replacement for your preferred shell environment. For complex shell workflows, interactive sessions, or shell-specific features, continue using your native ZSH, PowerShell, or bash terminal alongside Forge.
:::
