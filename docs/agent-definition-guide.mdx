...
#### Option 1: Global Directory (Recommended)
...
#### Option 2: Project Specific Directory
*Higher priority - project agents override global agents with the same ID.*
...
# PROMPTS
system_prompt: "You are a backend development expert specializing in APIs and server architecture."
user_prompt: |
...
max_tool_failure_per_turn: 3      # Max tool failures before forcing completion
max_requests_per_turn: 10         # Max requests in a single turn
...
- **Note**: Must use dot prefix (`.forge`) for project directories
- Higher priority - overrides global agents with same ID
...
#### Option 1: Project Specific Directory (Recommended)
...
#### Option 2: Global User Profile Directory
Create the agents directory in your user profile. **Note**: No dot prefix required for the global directory:
